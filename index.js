...
inquirer
    .prompt([
        ...
        {
            type: 'input',
            message: 'What version of League of Legends do you want to use?',
            name: 'version',
            default: 'latest'
        },
        {
            type: 'checkbox',
            message: 'Select the references you want to generate the list from.',
            name: 'include',
            choices: [
                {
                    name: 'Champions',
    ...
    .then(async (answers) => {
        ...
        if (include.includes('summonerspells')) {
    ...
    .catch((error) => {
        if (error.isTtyError) {
            return log(chalk.red('E'), 'Prompt couldn\'t be rendered in the current environment.');
        } else {
            return log(chalk.red('E'), 'Something went wrong.');
        }
    });
